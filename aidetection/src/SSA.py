...
class Head(nn.Module):
    def __init__(self, num,inchanl=3):
        super(Head, self).__init__()
        stem = [nn.Conv2d(inchanl, 64, 7, 2, padding=3, bias=False), nn.BatchNorm2d(64), nn.ReLU(True)]
        for i in range(num):
...
class ShuntedTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3,num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, num_conv=0,inchanl=3):
        super().__init__()
        ...
        for i in range(num_stages):
            if i ==0:
                patch_embed = Head(num_conv,inchanl=inchanl)#
            else:
...
def shunted_s(pretrained=False,**kwargs):
    model = ShuntedTransformer(
        patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 4, 12, 1], sr_ratios=[8, 4, 2, 1], num_conv=1, inchanl=3,**kwargs)
    model.default_cfg = _cfg()
    return model
...
def shunted_b(pretrained=False,**kwargs):
    model = ShuntedTransformer(
        patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 24, 2], sr_ratios=[8, 4, 2, 1], num_conv=2,inchanl=3,
        **kwargs)
    model.default_cfg = _cfg()
    return model
...
# @register_model
# def shunted_s(pretrained=False,mode='rgb',**kwargs):
#     if(mode=='rgb'):
#         model = ShuntedTransformer(
#             patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
#             norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 4, 12, 1], sr_ratios=[8, 4, 2, 1], num_conv=1, inchanl=3,**kwargs)
...
#         model = ShuntedTransformer(
#             patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
#             norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 4, 12, 1], sr_ratios=[8, 4, 2, 1], num_conv=1,inchanl=1,**kwargs)
#         model.default_cfg = _cfg()
#     return model


# @register_model
# def shunted_b(pretrained=False,mode='rgb',**kwargs):
#     if(mode=='rgb'):
#         model = ShuntedTransformer(
#             patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
#             norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 24, 2], sr_ratios=[8, 4, 2, 1], num_conv=2,inchanl=3,
...
#         model = ShuntedTransformer(
#             patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
#             norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 24, 2], sr_ratios=[8, 4, 2, 1], num_conv=2,inchanl=1,
#             **kwargs)
#         model.default_cfg = _cfg()
#     return model
